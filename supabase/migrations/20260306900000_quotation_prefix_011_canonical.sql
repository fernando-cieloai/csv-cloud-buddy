-- Match phone_rates.prefix to country_regions.region_code ignoring international 011-style prefixes
-- (same rules as src/lib/quotationPrefixCanonical.ts)
...
DECLARE
  n text;
BEGIN
  n := TRIM(BOTH '+' FROM TRIM(COALESCE(p, '')));
  IF n = '' THEN
    RETURN '';
  END IF;
  ...
  END LOOP;
  IF length(n) > 3 AND n LIKE '11%' THEN
