-- Normalize country_filter_key from network + country using the same rules as the app:
-- hyphen segment first (MX-TELCEL → mx), else leading ISO-like AA when followed by space/hyphen (AD FIXED → ad),
-- else first word (COLOMBIA MOBILE → colombia).

CREATE OR REPLACE FUNCTION quotation_country_filter_token(p_network text, p_country text)
RETURNS text
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
AS $$
  SELECT lower(trim(
    COALESCE(
      NULLIF(
        CASE
          WHEN NULLIF(trim(COALESCE(p_network, '')), '') IS NOT NULL THEN
            CASE
              WHEN position('-' IN trim(COALESCE(p_network, ''))) > 0 THEN
                trim(split_part(trim(COALESCE(p_network, '')), '-', 1))
              WHEN trim(COALESCE(p_network, '')) ~ '^[A-Za-z]{2}(\s|-)' THEN
                substr(trim(COALESCE(p_network, '')), 1, 2)
              ELSE
                trim(split_part(trim(COALESCE(p_network, '')), ' ', 1))
            END
          ELSE NULL::text
...
AS $$
  ...
  with_network AS (
    SELECT
      ...
      quotation_country_filter_token(pr.network, pr.country) AS country_filter_key
    FROM phone_rates pr
...
AS $$
  SELECT y.label
  FROM (
    SELECT DISTINCT quotation_country_filter_token(trim(pr.network), trim(pr.country)) AS label
    FROM phone_rates pr
    WHERE pr.upload_id = ANY(p_upload_ids)
  ) y
