-- Country filter: match ISO-style network prefixes (MX, AR) together with English names,
-- and allow longer labels to match as substrings of network (e.g. Argentina in …-ARGENTINA-…).
-- Filter labels RPC: prefer first segment of network (country token), fall back to country column.
        ...
        OR EXISTS (
          ...
          AND (
            ...
            OR (
              ...
              AND (
                LOWER(TRIM(SPLIT_PART(TRIM(pr.network), '-', 1))) = LOWER(TRIM(l.lbl))
                OR (
...
AS $$
  SELECT y.label
  FROM (
    SELECT DISTINCT trim(both from seg)::text AS label
    FROM (
      SELECT NULLIF(TRIM(SPLIT_PART(TRIM(pr.network), '-', 1)), '') AS seg
      FROM phone_rates pr
      WHERE pr.upload_id = ANY(p_upload_ids)
        AND NULLIF(TRIM(pr.network), '') IS NOT NULL
      UNION
      SELECT NULLIF(TRIM(pr.country), '') AS seg
      FROM phone_rates pr
      WHERE pr.upload_id = ANY(p_upload_ids)
        AND (
          NULLIF(TRIM(pr.network), '') IS NULL
          OR NULLIF(TRIM(SPLIT_PART(TRIM(pr.network), '-', 1)), '') IS NULL
        )
    ) x
